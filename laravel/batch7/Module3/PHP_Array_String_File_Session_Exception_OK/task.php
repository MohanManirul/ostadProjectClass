...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_POST['task']) && !empty(trim($_POST['task']))){ 
        ...
        exit;
    }elseif(isset($_POST['delete'])){ // ফাংশনটি PHP-তে কোনো ভেরিয়েবল সেট আছে কিনা এবং তার মান NULL নয় কিনা তা চেক করতে ব্যবহৃত হয়।
                                        // এটি যদি ভেরিয়েবলটি সেট থাকে এবং NULL না হয়, তাহলে true রিটার্ন করে, নতুবা false রিটার্ন করে।
        // TODO: Handle task deletion (check if a delete action is requested, remove the task from the array, and save)
        // delete a task
        unset($tasks[$_POST['delete']]); // ফাংশনটি একটি ভেরিয়েবল বা অ্যারে ইলিমেন্টকে মুছে ফেলার জন্য ব্যবহৃত হয়।
        $tasks = array_values($tasks); // অ্যারের সমস্ত মান (values) বের করে, এবং ইন্ডেক্স পুনরায় ধারাবাহিকভাবে রিনিউ করে।
        saveTasks($tasks);
        header('Location: ' . $_SERVER['PHP_SELF']);
        exit;
